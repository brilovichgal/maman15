    ...
    def updateMutexActions(self, previousLayerMutexProposition):
        ...
        for a1 in currentLayerActions:
            for a2 in currentLayerActions:
                if (a1 != a2) \
                        and (Pair(a1, a2) not in self.actionLayer.getMutexActions()) \
    ...
    def updatePropositionLayer(self):
        ...
        for action in currentLayerActions:
            for proposition in action.getAdd():
                if proposition not in self.propositionLayer.getPropositions():
                    self.propositionLayer.addProposition(proposition)
                if action not in proposition.getProducers():
                    proposition.addProducer(action)
    ...
    def updateMutexProposition(self):
        ...
        for prop1 in currentLayerPropositions:
            for prop2 in currentLayerPropositions:
                if prop1 != prop2 \
                        and Pair(prop1, prop2) not in self.propositionLayer.getMutexProps() \
                        and mutexPropositions(prop1, prop2, currentLayerMutexActions):
